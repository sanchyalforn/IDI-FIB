...
vec3 colFocus1 = vec3(0.8, 0.8, 0.8);
vec3 llumAmbient1 = vec3(0.2, 0.2, 0.2);
vec3 posFocus1 = vec3(1, 1, 1);  // en SCA
...
vec3 Lambert2 (vec3 NormSCO, vec3 L) 
{
    ...
    // Inicialitzem color a component ambient
    vec3 colRes = llumAmbient1 * matamb;

    // Afegim component difusa, si n'hi ha
    if (dot (L, NormSCO) > 0)
      colRes = colRes + colFocus1 * matdiff * dot (L, NormSCO);
    return (colRes);
}
...
vec3 Phong2 (vec3 NormSCO, vec3 L, vec4 vertSCO) 
{
    ...
    // Afegim la component especular
    float shine = pow(max(0.0, dot(R, V)), matshin);
    return (colRes + matspec * colFocus1 * shine); 
}
...
{	
    ...
    if (test == 1){
        ...
        vec4 posFocusSCO = view * vec4(posFocus1, 0.0);
